...
* Obtain the GIT structure (as [.zip](https://github.com/shsingh/sase/archive/master.zip) or use [GIT](https://github.com/shsingh/sase.git))
* Change to the sase directory, then build:
```bash
docker-compose -p sase build
```
...
```bash
docker-compose -p sase up
```
* Initialize OpenVPN CA Example (has to run interactively)
```bash
docker run --rm -ti --volume=sase_data:/data --volume=sase_openvpn_priv:/data-priv sase_openvpn --init=udp://vpn.example.com:5443
```
...
```bash
docker run --rm -ti --volume=sase_data:/data --volume=sase_squid_priv:/data-priv sase_squid --getca
```
...
```bash
docker run -ti --rm --volume=sase_data:/data --volume=sase_openvpn_priv:/data-priv sase_openvpn --getclient=<client_cn>
```
...
* SSL bump (man in the middle) can be disabled for defined sites by modifying /data/squid/nobump.txt.
    * The file is located on the Docker 'sase_data' volume
...
```bash
docker rm sase_clamav_1 sase_squid_1 sase_cicap_1 sase_openvpn_1 sase_dnsmasq_1
